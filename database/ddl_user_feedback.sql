...
CREATE TABLE IF NOT EXISTS `t_user_feedback` (
    ...
    `feedback_type` VARCHAR(20) NOT NULL COMMENT '反馈类型：BUG_ISSUE-问题/BUG，FEATURE_REQUEST-功能建议，CONTENT-内容质量问题，ACCOUNT-账号问题，OTHER-其他',
    `feedback_source` VARCHAR(20) NOT NULL DEFAULT 'APP' COMMENT '反馈来源：APP-APP内，MINI_PROGRAM-小程序，WEB-网页，H5-移动H5，OFFLINE-线下',
    `title` VARCHAR(100) NOT NULL COMMENT '反馈标题',
    `content` TEXT NOT NULL COMMENT '反馈内容',
    `contact` VARCHAR(100) NULL COMMENT '联系方式（手机号/邮箱/微信号等）',
    `is_contactable` TINYINT NOT NULL DEFAULT 1 COMMENT '是否允许联系：0-否 1-是',
    `status` VARCHAR(20) NOT NULL DEFAULT 'PENDING' COMMENT '处理状态：PENDING-待处理，IN_PROGRESS-处理中，RESOLVED-已解决，REJECTED-已驳回',
    `priority` VARCHAR(10) NOT NULL DEFAULT 'MEDIUM' COMMENT '优先级：LOW-低，MEDIUM-中，HIGH-高',
